...
Same as `ui-functionality`: fix clear frontend/backend wiring bugs (missing dosage-clear call, wrong state variable, dropped disclaimer append, model routing bug) directly; re-run the relevant command from above before reporting done. Hand off explanation-quality-only findings to [`/quality-test`](../quality-test/SKILL.md) — don't try to grade prose quality here.
